...
def main():
    ...
    for cause in fire_cause_over_time_values:
        fire_cause_over_time_values[cause] = \
                                    st.sidebar.checkbox(data_processing.CAUSE_REFERENCES[cause], 
                                                        True)
    ...
    for lightning causes on the yearly scale, but the similarities begin to
    ...
    st.write('''Before we get into modelling and making predictions for
    future forest fires, we need to analyse the factors that are most closely
    associated with the start of a forest fire. According to the Center for
    Climate and Energy Solutions, wildfire risk can depend on temperature,
    presence of trees and shrubs, and soil moisture. Given the data we've
    collected so far, we've decided to organize these factors into two main
    groups: Temperature and Precipitation (where precipitation is rain or
    snow).''')
    # SOURCE
    ...
    increases the chances for larger fires to occur.''')
    ...
    future_weather_temperature = st.slider('Temperature Offset', -2.0, 2.0, 1.6)
    future_weather_precipitation = st.slider('Precipitation Multiplier', -10.0, 10.0, 5.5)
    ...
    st.line_chart(fires_2019_data['# of fires'])
    st.line_chart(future_fires_data['# of fires'])
    #future_fires_per_month_graph = px.line(future_fires_data, x='MONTH', y='# of fires', color='YEAR')
    #st.plotly_chart(future_fires_per_month_graph)
